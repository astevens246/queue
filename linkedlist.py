...
class LinkedList(object):
    ...
    def append(self, item):
        ...
        Best and worst case running time: O(1) because we track the tail"""
    ...
    def prepend(self, item):
        ...
        Best and worst case running time: O(1) because we just update head"""
        # Create a new node to hold the given item
        new_node = Node(item)
        # Check if this linked list is empty
        if self.is_empty():
    ...
    def delete_head(self):
        """Delete and return the head node's data, or raise ValueError if empty."""
        if self.is_empty():
